...
export default function AuthScreen() {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            // Extract token and user from the nested response structure
            ...
            // Role validation - only allow GUARD and SUPER_ADMIN
            const allowedRoles = ['GUARD', 'SUPER_ADMIN'];
            if (!allowedRoles.includes(user.role)) {
                console.error('❌ Unauthorized role:', user.role);
                setError(
                    `Access denied. This app is for guards only.\n` +
                    `Your role: ${user.role}\n` +
                    `Please use the ${user.role.toLowerCase()} app instead.`
                );
                setIsLoading(false);
                return;
            }

            console.log('✅ Role validated:', user.role);
            ...
            if (err.response?.status === 401) {
                ...
                setError('Access denied. Please verify your guard credentials.');
            } else if (err.response?.status === 404) {
                setError('Login service unavailable. Please contact your administrator.');
    ...
    return (
        <View className="flex-1">
            {/* Blue Gradient Background - Matching Main App */}
            <LinearGradient
                    ...
                    >
                        <View className="flex-1 px-6 justify-center pb-24">
                            ...
                            >
                                ...
                                <Text className="text-slate-500 mb-6">Sign in to start your shift</Text>
