...
export default function DailyHelpTypeList() {
  ...
  const typeInfo = DAILY_HELP_TYPES.find(t => t.type === type);
  let filtered = DAILY_HELPERS.filter(h => h.type === type && h.name.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>{typeInfo?.label ?? type}</Text>
        <View style={{ width: 22 }} />
      </View>
